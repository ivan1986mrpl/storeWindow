...
const changeModalState = state => {
  ...
  (0,_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('#width'); //(чтобы в поле ввода инпута можно было ввести только числа)
  (0,_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('#height');
  function bindActionToElems(event, elem, prop) {
    //(event = событие, elem = элемент, на котором происходит, )
    elem.forEach((item, i) => {
      item.addEventListener(event, () => {
        switch (item.nodeName) {
          case 'SPAN':
            //(когда мы используем свойство nodeName, нам приходят названия элементов в верхнем регистре)
            state[prop] = i;
            break;
          case 'INPUT':
...
const checkNumInputs = selector => {
  ...
  numInputs.forEach(item => {
    //(чтобы в поле ввода инпута можно было ввести только числа)
...
const forms = state => {
  ...
  (0,_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('input[name="user_phone"]'); //(чтобы в поле ввода телефона можно было ввести только числа)
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      e.preventDefault(); //(отменяем поведение т.к. форма при отправке перезагружается)
...
const images = () => {
  ...
  workSection.addEventListener('click', e => {
    ...
    if (target && target.classList.contains('preview')) {
      //(если пользователь кликнул в картинку, а не между ними)
...
const modals = () => {
  function bindModal(triggerSelector, modalSelector, closeSelector, closeClickOverlay = true) {
    const trigger = document.querySelectorAll(triggerSelector),
      ...
      //(дата атрибут в квадратных скобках)
      scroll = calcScroll();
    ...
    modal.addEventListener('click', e => {
      //(чтобы окно закрывалось при клике на подложку)
      if (e.target === modal && closeClickOverlay) {
        //(если пользователь кликнул на подложку closeClickOverlay == true)
        windows.forEach(item => {
          //(закрываем все открытые модальные окна)
          item.style.display = 'none';
        });
  ...
  function showModalByTime(selector, time) {
    //(показ модального окна, если пользователь time секунд на странице)
    setTimeout(function () {
  ...
  function calcScroll() {
    //(чтобы страница не прыгала при открытии модального окна на ширину полосы прокрутки)
...
const tabs = (headerSelector, tabSelector, contentSelector, activeClass, display = 'block') => {
  ...
  header.addEventListener('click', e => {
    //(определяем на какой таб кликнул пользователь)
    const target = e.target;
    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      //(tabSelector.replace(/\./, '') чтобы убрать точку из имени класса)
      tab.forEach((item, i) => {
...
const timer = (id, deadline) => {
  const addZero = num => {
    //(чтобы подставлялся 0 впереди одиночной цифры в таймере)
    if (num <= 9) {
      return '0' + num;
    } else {
      return num;
  ...
  const getTimeRemaining = endtime => {
    const t = Date.parse(endtime) - Date.parse(new Date()),
      //(разница между дедлайном и тем временем, которое у нас сейчас)
      seconds = Math.floor(t / 1000 % 60),
      minutes = Math.floor(t / 1000 / 60 % 60),
      //((общее время в миллисекундах делим на 1000 и делим на 60) и это значение делим с остатком на 60)
  ...
  const setClock = (selector, endtime) => {
    ...
    function updateClock() {
      ...
      if (t.total <= 0) {
        //(чтобы избежать утечек памяти и остановить таймер после дедлайна)
        days.textContent = "00";
...
/* harmony import */ var slick_carousel__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(slick_carousel__WEBPACK_IMPORTED_MODULE_1__);
 //импорт из папки node modules, установил npm i jquery --save
 // импорт из папки node modules, установил npm i slick-carousel
...
window.addEventListener('DOMContentLoaded', () => {
  //(глобальный обработчик события DOMContentLoaded отвечает за то, что скрипты начинают выполняться на странице тогда, когда DOM структура готова)
  "use strict";

  let modalState = {};
  let deadline = '2024-02-18';
  (0,_modules_changeModalState__WEBPACK_IMPORTED_MODULE_4__["default"])(modalState); //(объект с данными из калькулятора)
